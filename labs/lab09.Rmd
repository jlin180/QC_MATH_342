...
for(i in 1:length(num_trees)){
  rf_mod = randomForest(price ~ ., data = diamonds_samp, ntrees = num_trees[i])
...
for( i in 1:length(mtrys)){
  ...
  oob_ME_by_mtrys[i] = sd(adult_samp$income != rf_mod$predicted)
}
...
random_bagged_ols = function(X, y, num_ols_models = 100, mtry = NULL){
  ...
  mtry = sample(colname, floor((ncol(X)-1)/2))
  ols = lm(y ~ ., data = X[,mtry])
  ols_bag[[i]] = ols
  }
  ols_bag
}
...
```{r}
punch_holes = function(X, prob_missing){
  X = as.matrix(X)
  X[sample(1:length(X), length(X)*prob_missing)] <- NA
  X = data.frame(X)
  X
}
